...
export const decrypt = (inputText, userKey) => {
    const base64Decoded = CryptoJS.enc.Base64.parse(inputText).toString(CryptoJS.enc.Utf8);
    const key = userKey + keySalt;
    const decrytedObj = CryptoJS.AES.decrypt(base64Decoded, key).toString(CryptoJS.enc.Utf8);
